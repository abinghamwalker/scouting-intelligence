# W04 Wyscout v5 canonical schema and deterministic rebuild design — R16

Status: **implementation design for master and independent review; not self-approved**

This document replaces R15 in full. It is the standalone design for the local-only
W04 Wyscout Figshare v5 Bronze-to-Gold proof. R16 retains every passing,
master-reproduced R15 closure, every earlier passing closure, and every passing
independent R9 finding without compression or substitution: all source,
rights, temporal, identity, football-product, coverage, quarantine, path,
serializer, environment, resource, exact executable/alias/bytecode census, gate,
ownership, two-root, and two-commit ledger contracts remain binding. R15 received
the master candidate decision recorded by its review, then independent R9 returned
`REWORK` for one reproduced P1. Neither decision is retroactively changed. R16
corrects only that returned defect: all outer and child normal `PATH` resolution
now records the exact logical `/opt/homebrew/bin/uv` launch spelling in `UV`, while
an exact one-hop symlink admission constructively binds that spelling to the
already admitted uv 0.9.21 physical bytes. The R15 acyclic outer/child environment,
exhaustive child inputs/results, canonical transport, chronology, 25-key
projection, and bounded-scope closures remain unchanged, as do the R13
encoding-source,
descriptor/race, result-v2, exact argv, 58/19-plus-four-predicate pyc,
35-executable, three-alias, three-`.pth`, and 17-resource closures. It changes no
architecture, provider, rights, dependency, lock, migration, network,
storage-root, ignore, remote, deployment, product writer, or child argv.
...
The accepted `w04-local-control-bootstrap-v3` tuple is exactly:
...
uv_logical_launch_token = <W04_UV_LOGICAL_LAUNCH_PATH>
uv_logical_lstat_type = symlink
uv_logical_raw_target = ../Cellar/uv/0.9.21/bin/uv
uv_logical_raw_target_size = 26
uv_logical_resolution_hops = 1
uv_physical_path = /opt/homebrew/Cellar/uv/0.9.21/bin/uv
...
The uv authority is constructive and exact. `/opt/homebrew/bin/uv` must `lstat` as
one symlink whose uninterpreted `readlink` bytes are exactly
`../Cellar/uv/0.9.21/bin/uv` (26 bytes). Resolving that raw target relative to the
logical link's parent must take exactly one hop contained within `/opt/homebrew`
and end, without another link or normalization choice, at the regular non-symlink executable
`/opt/homebrew/Cellar/uv/0.9.21/bin/uv`. That final file must have mode `0o555`,
size 41,617,552, the exact SHA-256 and version above, and byte equality with the
already admitted physical uv authority. The logical link path, raw target, exact
one-hop relationship, final physical path, mode, size, digest, and version are
stable. Runtime lstat/readlink/stat device, inode, link-count, and clock
observations are operational. Missing or non-symlink logical entry, raw-target
drift, an extra hop, cycle, escape, alternate logical path, unequal final target,
or physical byte/version/mode/size drift fails before process creation.
The exact version observation is obtained only through normal execution of the
literal `uv` token resolving to `/opt/homebrew/bin/uv`; the physical Cellar path
is never an exec target.
...
Its normalized substituted entries are exactly: eight operational values plus the
one stable logical uv launch value:
...
UV = /opt/homebrew/bin/uv; stable token
...
component is exactly `/opt/homebrew/bin` (stable token `<W04_UV_BIN_DIR>`, not the
...
the tuple. This transformation algorithm and its positive exact-uv test are stable
authority; only the project-root-bearing `PATH`, prefix, and venv actual values
remain operational.
...
the nine named substitutions (eight operational and the one exact stable logical
uv launch substitution), the exact deterministic uv
...
4. Insert that digest into `w04-local-control-bootstrap-v3`, canonical-encode the
...
eight outer normalized entries with the same accepted actual values (seven
operational values plus the exact stable logical uv value):
...
Python actually receives, while the stable process-launch contract also binds the
exact map transported into uv and the exact deterministic transformation between
them. Either child disagreeing with its input map, with the outer map, or with the
other child fails.
...
| `bootstrap_tuple_sha256` | string | SHA-256 of decoded canonical `w04-local-control-bootstrap-v3` bytes, including the exact logical-link-to-physical-uv proof; stable |
| `code_manifest_schema_version` | string | exact `w04-code-environment-admission-v13`; stable |
...
| `manifest_schema_version` | string | exact `w04-code-environment-admission-v13` |
...
SHA-256, and the exact uv authority: logical launch path
`/opt/homebrew/bin/uv`, raw one-hop target
`../Cellar/uv/0.9.21/bin/uv`, final physical path
`/opt/homebrew/Cellar/uv/0.9.21/bin/uv`, version, mode, size, and physical
executable SHA-256. The normalized environment digest substitutes only
`<W04_UV_LOGICAL_LAUNCH_PATH>` for the logical `UV` value; it never substitutes a
physical-path token or accepts both spellings. Runtime device/inode/link-count and
clock observations are recorded only in the operational admission receipt and are
excluded from semantic and build identity.
...
stable code/environment manifest:
  ...
  exact /opt/homebrew/bin/uv logical launch path, symlink lstat kind, raw
    ../Cellar/uv/0.9.21/bin/uv target, one-hop contained resolution, and equality
    to the admitted physical uv path/version/mode/size/digest
...
operational admission evidence:
  ...
  actual uv logical-link and final-file device/inode/link-count/clock observations
...
The stable manifest algorithm is `w04-code-environment-admission-v13`. Component
...
environment_digest = SHA256(canonical_json({
  ...
  "uv_physical_sha256": <exact admitted physical executable bytes digest, constructively reached from the one exact logical path/raw-link/one-hop authority>,
  "local_launcher_control_digest": <launcher row, role, exact argv, exact logical-link-to-physical uv tuple, acyclic w04-outer-environment-bootstrap-v2 construction, completed bootstrap tuple, encoding-source verifier, control-prefix policy, and inherited launcher-source descriptor policy>,
  "process_launch_contract_digest": <the exact outer and two exact child ordered locked/no-sync argv, normal PATH selection of only the logical uv path, uv-input maps and admitted logical-spelling-preserving uv input-to-Python environment transformation, direct-physical/either-spelling/post-hoc-realpath denials, three roles, unique chronology, three prefix policies, source-descriptor code rows, closed child environments, w04-child-input-v1 transport, the exact w04-wyscout-pre-build-projection-v1 schema/algorithm, and the post-hash w04-rebuild-invocation-v1 schema; never a completed projection or invocation instance>,
...
`/opt/homebrew/bin/uv -> ../Cellar/uv/0.9.21/bin/uv` one-hop authority or its
exact final admitted physical identity. Operational evidence cannot be substituted
into a stable component.
...
`w04-code-environment-admission-v13` bytes have been written or confirmed,
...
- `code_manifest_id` and `code_manifest_sha256` bind the complete immutable
  ...
  pyc authority and four orphan predicates, the exact logical-link-to-physical uv
  executable evidence, launcher and exact process/result schema authority, three
...
alias/raw-link/physical-path spellings other than the exact stable uv logical-link
authority, root-bearing project executable paths/bytes,
...
- exact `/opt/homebrew/bin/uv` logical selection and `UV` value in the outer and
  both child maps, the raw `../Cellar/uv/0.9.21/bin/uv` symlink target, exactly
  one contained resolution hop to the same admitted physical uv
  path/version/mode/size/digest, with no direct physical execution or alternate
  accepted spelling;
...
cache paths, uv device/inode/link-count/clock observations, actual
...
`/opt/homebrew/bin/uv`, the exact logical `UV` value in all three maps, the
...
| 21L | local-control implementer/master | future exact `scripts/launch_wyscout_v5.py` plus acyclic bootstrap digest, first-instruction guard, whole-repo pyc census, source-descriptor/closed child-input/result/diagnostics, three-prefix chronology, TOCTOU, manifest-write, exact pre-build projection, one build-ID SHA-256, and post-hash invocation tests | transient control only; R16 names but does not create script |
| 22 | admission implementer/master | future exact `scripts/admit_wyscout_v5_runtime.py` plus tests: locked/no-sync no-site/Packaging bootstrap, pre-build admission prefix/process, closed input envelope, `.pth`/editable root, `L==I`, 35 executables, three aliases, source-complete pyc authority/classes, canonical manifest construction/result frame | offline constructor only; cannot write manifest/calculate build ID; R16 does not create script |
| 23 | rebuild entrypoint/master | future exact `scripts/rebuild_wyscout_v5.py`, locked/no-sync integration test, exact rebuild-input envelope, bounded result frame, projection reconstruction, invocation receipt only | only after manifest readback, one build-ID SHA-256, post-hash invocation construction, then rebuild-prefix creation; calls sole writers; R16 does not create script |
...
15. Prove the exact logical launch path, raw one-hop symlink target, final admitted
    physical path, uv version, physical mode/size/digest, and sole logical
    environment token are stable; only runtime device/inode/link-count/clock
    observations are operational. Assert the exact 17 resources and disjoint guard
    categories.
...
R16 retains every passing R15 closure and every passing independent R9 finding
without changing the recorded R15 master-candidate or R9 `REWORK` decisions and
without self-approval. R16 created or modified only this design and its assigned
return under the project root; it created no parent-workspace report path,
directory, future script, or cleanup obligation. Implementation begins only after
the master and a separate independent reviewer accept this standalone design.
