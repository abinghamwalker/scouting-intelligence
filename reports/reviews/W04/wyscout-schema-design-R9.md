# W04 Wyscout v5 canonical schema and deterministic rebuild design — R9

Status: **implementation design for master and independent review; not self-approved**

This document replaces R8 in full. It is the standalone design for the local-only
W04 Wyscout Figshare v5 Bronze-to-Gold proof. R9 retains every R8 closure without
compression or substitution, including R8's total unknown-kind quarantine and
conservative all-groups distribution closure and every restored R7/R6/R5 source,
temporal, identity, football-product, coverage, path, environment, resource, gate,
ownership, and ledger contract. It corrects only the master-measured executable
census, venv interpreter alias, no-site startup, uv-bootstrap/editable-root,
pre-existing bytecode, and compatible-tag bootstrap defects. It changes no
...
external admitted launcher verifies the stage-0 inputs below and starts exactly the
canonical venv interpreter with `-S -B`. The sole third-party pre-rebuild bootstrap
is the byte-admitted Packaging 26.2 selector described in Section 8.0. Its bounded
pyc opens are only the pre-rebuild magic/hash/size reads in Section 8.6; stage-0 and
rebuild read zero pyc bytes.

### 8.0 Constructive no-site stage-0 and compatible-tag bootstrap

The external launcher sets all fixed environment values, including
`PYTHONDONTWRITEBYTECODE=1` and an absolute `PYTHONPYCACHEPREFIX` naming the already
proved empty staging `runtime-pycache/`, before interpreter creation. It launches:

```text
uv run python -S -B <repository-owned-stage-0-module>
```

The `uv` executable, both venv interpreter aliases, physical interpreter, stage-0
repository bytes, environment, empty alternate prefix, and no-network controls are
already externally hashed/admitted. `-S` is mandatory. At first Python instruction,
`site` has not initialized, the site-packages and editable source roots are absent
from `sys.path`, `_virtualenv` and `coverage` are absent from `sys.modules`, and no
`.pth` has run. Any contrary observation fails. The stage-0 module installs the
open/read/mmap/execute/import audit guard before manually appending paths. It never
...
`sys.executable`. The exact venv scripts directory contains these two required
aliases:
...
Both alias paths are contained in the exact venv scripts directory, have lstat mode
`0o755`, are symlinks, have no cycle or extra hop, and resolve to the same frozen
regular physical Python 3.12.12 executable. A relative hop must remain in the
scripts directory; the only permitted absolute leaf is the exact admitted physical
interpreter above. Its mode is `0o755`, size `49,968`, and SHA-256
...
closure must equal Section 8.8. Another alias, raw target, target bytes, escape,
cycle, non-link intermediate, or version/mode mismatch fails.

`python` is the canonical uv POSIX wrapper-shebang alias.
`uv run python` operationally reports the admitted `python3` alias. Admission
records both absolute alias paths, raw link texts, lstat identities, complete
resolution chains, resolved physical path and exact wrapper shebang. Stable identity
binds alias basenames, safe-chain topology (`python3 -> python -> physical
interpreter`), physical interpreter identity/digest, and algorithm
`w04-venv-wrapper-interpreter-alias-v1`, never either root-bearing absolute alias
path or the external physical path spelling.
...
and W bytes. Admission may read all 35 only before denial. From stage-0 guard
installation through final recheck, open/read/mmap/execute of every executable
target is denied and audited. Zero reads and zero executions are required.

### 8.6 Complete pre-existing bytecode census, classes, and read denial

Pre-existing `__pycache__` directories and pyc are operational state, not code
authority. They are never deleted, imported, or used to select a source. Before the
Section 8.0 interpreter exists, the external launcher enumerates both the exact site
root and all admitted repository code roots, with safe contained lstat traversal and
no symlink following. It snapshots directory entries, file identity, hardlink count,
mode, size, SHA-256, first 16 header bytes, and magic. Any path escape, symlink,
hardlink alias, non-regular pyc, duplicate path, unreadable file, or mutation during
the bounded read fails.

The current site inventory is exactly 1,075 pyc. Its lexical filename split is 962
normal-cache names, 112 pytest assertion-rewrite names, and one source-absent
orphan. The 962 normal names refine to 961 distribution-owned sources plus one
uv-bootstrap pyc. These four authority classes are mutually exclusive:

1. `SITE_DISTRIBUTION_NORMAL` — 961 files named exactly
   `<source-stem>.cpython-312[.opt-0|.opt-1|.opt-2].pyc`. Standard-library
   `importlib.util.source_from_cache` must map each to one existing verified
   RECORD-owned `.py` in the same selected distribution. Cache tag is exactly
   `cpython-312`; optimization is absent or exactly 0/1/2.
2. `SITE_PYTEST_REWRITE` — 112 files named exactly
   `<source-stem>.cpython-312-pytest-9.1.1.pyc`. Construct the source path by
   removing only that complete suffix and appending `.py`; it must be one existing
   verified RECORD-owned source with one distribution owner. The suffix version
   must equal the admitted `pytest==9.1.1` distribution exactly. The source owner
   remains the file's authority; pytest authorizes only the reviewed suffix grammar.
   Another pytest version, stacked suffix, optimization suffix, or missing/ambiguous
   source fails.
3. `UV_BOOTSTRAP_NORMAL` — exactly
   `site-packages/__pycache__/_virtualenv.cpython-312.pyc`, mapped only to the exact
   Section 8.3A `_virtualenv.py`, with bootstrap—not distribution—ownership. Current
   mode is `0o644`, size `4,159`, and SHA-256
   `08765615dd291d8a643581c2e7a0d3f891284aed32dd38a3940675488579f5f6`.
4. `ORPHANED_INERT` — exactly
   `site-packages/__pycache__/six.cpython-312.pyc`, whose sibling source is absent
   and which has no RECORD/bootstrap owner. It is admitted only as current inert
   operational debris: mode `0o644`, size `41,388`, SHA-256
   `4e59431b1d92fe443cbdb1f76e065ece05b1c4f6cb4925168be8e9321f390e28`.
   No second orphan, alternative path, source appearing, or ownership claim is
   allowed. An orphan never enters a stable source map or import authority.

Every class requires current `importlib.util.MAGIC_NUMBER`, current interpreter
cache tag, safe containment, exact observed hash/size/mode, and zero read after
stage-0 begins. Normal/pytest/bootstrap rows bind their exact verified source
hash/size. The orphan binds only its operational path and bytes; it cannot satisfy an
import, owner, `L`, environment, or build input.

The admitted repository code roots have the master-verified current inventory of 56
pyc files in 17 `__pycache__` directories: 36 normal-cache names and 20 exact
`cpython-312-pytest-9.1.1` names. Apply the same normal and pytest suffix grammar,
but require each source to be one exact repo-manifest-owned `.py`; repository
ownership replaces RECORD ownership. Every repository pyc and cache directory is
operationally hashed/snapshotted and placed under the same denial. No pyc beneath a
repository path outside the admitted code-root manifest can be imported; any such
path is included in the whole-repository deny scan and reported as non-authoritative
operational state. It never expands the 56-row admitted repository source mapping.

Stable identity binds algorithm
`w04-preexisting-pyc-enumerate-deny-v2`, current cache tag/magic, both normal/pytest
grammars, exact admitted pytest version, bootstrap source authority, one-orphan
inert predicate, repository/site traversal roots, no-cleanup policy, and sorted
source maps:

```text
(authority-class, distribution-or-repository identity,
 source-relative path, source SHA-256/size,
 deterministic allowed normal/pytest cache-name grammar)
```

Actual pyc paths/bytes/hashes/sizes/modes/counts, orphan bytes, directory counts, and
inventory digests are operational evidence only and excluded from semantic
products, stable environment identity, build ID, and two-root equality. Thus two
roots may have different operational inventories but must have the same stable
policy/source authority.

Before stage-0 starts, the external launcher:

1. snapshots every current site and whole-repository pyc/`__pycache__` path;
2. proves the exact staging `runtime-pycache/` empty and sets its absolute
   `PYTHONPYCACHEPREFIX` before interpreter start;
3. sets `PYTHONDONTWRITEBYTECODE=1` and launches the exact interpreter with `-S -B`;
4. installs an audit/guard rule before adding site/repository roots, denying
   open/read/mmap/execute/import of every in-place pyc, every `.pth`, every Section
   8.5 executable, and any alternate bytecode path; and
5. after bootstrap and rebuild proves zero denied reads/executes/imports, no
   new/changed/deleted in-place pyc or cache directory, and an empty alternate
   prefix.

Any unsafe/class-ambiguous/wrong-tag/wrong-magic/wrong-pytest-version/source-mapping
failure, a second orphan, pyc read, `.pth` execution, executable read/execution,
new/changed bytecode, cache-directory mutation, or non-empty alternate prefix fails.
This retains R8/R6's enumerate-and-deny behavior while making the actual site and
repository inventories constructive without cleanup.
...
`I`, extracted/installed/35-row executable policy, complete site/repository pyc
policy/source maps, interpreter/resources, and ownership freeze may W04 execute.
...
The interpreter closure binds both exact safe venv aliases and raw link chains from
Section 8.5.1, canonical wrapper alias, launch-time alias observation,
implementation/full version, ABI, physical executable SHA-256/size/mode, loaded
libpython and loader images. Both aliases resolve to the same frozen physical
Python 3.12.12. Stable identity contains verified alias topology and
...
operational pyc, and hash exact regular source/extension bytes. Stage-0 is `-S -B`;
...
extracted/installed actual bytes, all 35 executables, complete site/repo pyc
no-read/no-change state, alias/interpreter, stdlib, environment, and 17 resources
are rechecked.
...
stable code/environment manifest:
  ...
  stable site/repository pyc policy/source maps
  uv version and physical executable digest
  wrapper-alias topology plus interpreter/libpython/loader/stdlib digests
...
operational admission evidence:
  ...
  actual python/python3 aliases, link text/chains and wrapper shebang
  ...
  no-site/Packaging observations and no-read/no-change audit
```

The stable manifest algorithm is `w04-code-environment-admission-v6`. Component
...
absolute path, cache absolute path, actual alias/shebang/executable path/hash,
...
ambiguous/read pyc, alias/interpreter/libpython/stdlib/resource drift, behavior-
environment drift, and early build-ID formation.

## 9. Build identity, deterministic bytes, and two-root proof

Canonical build-input version is `w04-wyscout-build-id-v7`. It contains:
...
- stable site/repository pyc policy/source maps, never operational root-bearing
  executable or pyc bytes/inventories;
- uv version and physical executable digest, never uv absolute path;
- exact safe wrapper-alias topology, interpreter/libpython/loader, and
...
output root, host clocks, Git branch/tag/commit, absent wheel archive bytes, and
output digests are excluded.
...
- stable site/repository pyc policy/source maps (not actual pyc bytes/inventories);
...
Operational run IDs/clocks, uv/cache paths, actual alias/link/shebang/executable
paths/hashes, actual root-bearing editable-install bytes/clocks, and actual site/repo
pyc inventories may differ only in their documented operational fields. They never
leak into stable identity.
...
census, complete site/repository bytecode classes and denial, exact resources, sole
writers, quarantine reconciliation, and two-root equality.
...
aliases; operational site/repository pyc class inventories and zero-read/no-change
proof; loaded subset `R`; uv/alias/interpreter/stdlib/resources; path ownership; and
two-root equality.
...
| 22 | admission implementer/master | admission module/script/tests including no-site/Packaging bootstrap, `.pth`/editable root, `L==I`, 35 executables, complete pyc classes | offline algorithm only |
...
9. Start stage-0 with `-S -B` and the empty alternate prefix already set; prove no
   site root, editable root, `_virtualenv`, coverage, `.pth`, executable, or pyc is
   observed. Byte-admit only Packaging 26.2, import only its verified RECORD-owned
   selector modules, freeze ordered tags once, and reject any extra import or
...
11. Prove both `python` and `python3` safe alias/link chains resolve to the exact
    physical Python 3.12.12 bytes/ABI/mode; require only canonical `python` in every
    wrapper shebang and reject incidental `sys.executable` spelling equivalence.
...
14. Enumerate pre-existing pyc without cleanup. Require the exact current site
    lexical split 962 normal/112 pytest-rewrite/one orphan, refine the bootstrap
    pyc, and cover 56 admitted repo pyc in 17 cache directories. Reject unsafe,
    class-ambiguous, wrong-tag/magic/pytest-version/source ownership, or a second
    orphan; bind operational hashes and stable site/repo source maps; prove early
    alternate prefix, zero reads/changes/new files, and empty alternate prefix.
...
R9 closes the returned R8 master findings without self-approval. Implementation begins
