# W04 23-root runtime constraint census R1

- Task: `W04-WYSCOUT-23-ROOT-RUNTIME-CONSTRAINT-CENSUS-01-R1`
...
## 1. Fixed-input readback

The packet-fixed inputs were checked before analysis. Every observed SHA-256 equalled
the packet binding.
...
| build/receipt audit R2 | `77cf8dcb79d202b960553dfd6da631af039e5ec0a382a0e7c30be770045d0491` |
| build/receipt audit R4 | `a6f8f3321dcfdb0c04d231d3e07d06497441ce703716d6e509f3f45b8829c222` |
...
The recursive union is the logical evidence type. It is not an Arrow union
authority.
...
- a field whose logical type is `CanonicalJsonValue` is one non-null Arrow UTF-8
  scalar containing exact canonical tagged logical JSON without LF; Arrow null is
  allowed only for outer logical absence;
...
| --- | --- | --- | --- | --- | --- |
| Bronze known | `raw_record.value[*].value` recursively | none | `raw_record.value`, `measured_raw_fields`, lineage sequences | number arms under the tagged path are encoded inside UTF-8 | `L` |
| Bronze rejected record | `raw_record.value[*].value` recursively; `raw_kind.value` | none | `raw_record.value`, lineage sequences | number arms under tagged paths | `L` |
| Bronze rejected field | `original_value` | none | lineage sequences | number arm is inside tagged UTF-8 | `L` |
...
`CanonicalJsonObject` itself is a named struct (`kind`, `value`); its member
`value` is the recursive `CanonicalJsonValue` leaf that must use tagged UTF-8.
Present canonical JSON null is therefore the non-null UTF-8 tagged null object, not
Arrow null. Optional outer fields elsewhere retain their separate nullable state.
