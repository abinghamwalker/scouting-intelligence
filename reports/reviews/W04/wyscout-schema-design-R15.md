# W04 Wyscout v5 canonical schema and deterministic rebuild design — R15

Status: **implementation design for master and independent review; not self-approved**

This document replaces R14 in full. It is the standalone design for the local-only
W04 Wyscout Figshare v5 Bronze-to-Gold proof. R15 retains every passing,
master-reproduced R13 and R14 closure, every earlier passing closure, and the
returned independent R8 findings without compression or substitution: all source,
...
ownership, two-root, and two-commit ledger contracts remain binding. R13 received
a master `REWORK` decision; R14 closed the defects returned from R13 but also
received a master `REWORK` decision. Neither revision is called accepted here and
neither decision is retroactively changed. R15 corrects only the defects returned
from R14: the build-ID/rebuild-invocation digest cycle, the rebuild dependency-row
aliases, and the R13/R14 acceptance-lineage wording. The R14 acyclic outer/child
environment, exhaustive child inputs/results, canonical transport, chronology,
and bounded-scope closures remain unchanged, as do the R13 encoding-source,
...
| `code_manifest_schema_version` | string | exact `w04-code-environment-admission-v12`; stable |
...
| `manifest_schema_version` | string | exact `w04-code-environment-admission-v12` |
...
The stable manifest algorithm is `w04-code-environment-admission-v12`. Component
...
`w04-code-environment-admission-v12` bytes have been written or confirmed,
...
- `code_manifest_id` and `code_manifest_sha256` bind the complete immutable
  ...
  pyc authority and four orphan predicates, uv executable evidence, launcher and
  exact process/result schema authority, three aliases, interpreter/stdlib, and
  fixed environment;
...
alias/raw-link/physical-path spellings, root-bearing executable paths/bytes,
actual `.pth`/editable metadata paths and `uv_cache` clocks, actual pyc
inventories/bytes/counts, uv/cache absolute paths and opaque cache keys, absent
...
| 21L | local-control implementer/master | future exact `scripts/launch_wyscout_v5.py` plus acyclic bootstrap digest, first-instruction guard, whole-repo pyc census, source-descriptor/closed child-input/result/diagnostics, three-prefix chronology, TOCTOU, manifest-write, exact pre-build projection, one build-ID SHA-256, and post-hash invocation tests | transient control only; R15 names but does not create script |
| 22 | admission implementer/master | future exact `scripts/admit_wyscout_v5_runtime.py` plus tests: locked/no-sync no-site/Packaging bootstrap, pre-build admission prefix/process, closed input envelope, `.pth`/editable root, `L==I`, 35 executables, three aliases, source-complete pyc authority/classes, canonical manifest construction/result frame | offline constructor only; cannot write manifest/calculate build ID; R15 does not create script |
| 23 | rebuild entrypoint/master | future exact `scripts/rebuild_wyscout_v5.py`, locked/no-sync integration test, exact rebuild-input envelope, bounded result frame, projection reconstruction, invocation receipt only | only after manifest readback, one build-ID SHA-256, post-hash invocation construction, then rebuild-prefix creation; calls sole writers; R15 does not create script |
...
R15 retains the passing, master-reproduced closures from the master-returned R13
and R14 revisions and the returned independent R8 findings without calling R13 or
R14 accepted and without self-approval. R15 created or modified only this design
and its assigned return under the project root; it created no parent-workspace
report path, directory, or cleanup obligation. Implementation begins only after
