# W04 Wyscout v5 canonical schema and deterministic rebuild design — R19

Status: **implementation design for master and independent review; not self-approved**

This document replaces R18 in full. It is the standalone design for the local-only
W04 Wyscout Figshare v5 Bronze-to-Gold proof. The master accepted R18; that
acceptance is preserved. The later independent R11 review is invalid and issued no
R18 merits verdict because its installed-Packaging helper created eleven mapped
normal pycs before the required census. R19 corrects only the resulting operational
evidence and closes the no-write review harness. It does not reinterpret R18,
retroactively validate R11, or turn reviewer-created caches into stable authority.
Every passing, master-reproduced R18 closure, every earlier passing closure, and
every valid passing independent finding remains binding without compression or
substitution: all source, rights, temporal, identity, football-product, coverage,
quarantine, path, serializer, environment, resource, exact executable/alias/
bytecode, gate, ownership, two-root, and two-commit ledger contracts remain
binding. In particular, the exact 119-pair field roster, strict UUID `ActorId`,
complete possession contract, and approved
`tests/contracts/test_wyscout_field_registry_authority.py` path remain unchanged.

All passing R18 closures remain unchanged: the current host's exact uv logical
...
17-resource closures remain binding. R19 changes no architecture, provider,
...
| 21L | local-control implementer/master | future exact `scripts/launch_wyscout_v5.py` plus acyclic bootstrap digest, first-instruction guard, whole-repo pyc census, source-descriptor/closed child-input/result/diagnostics, three-prefix chronology, TOCTOU, manifest-write, exact pre-build projection, one build-ID SHA-256, and post-hash invocation tests | transient control only; R19 names but does not create script |
| 22 | admission implementer/master | future exact `scripts/admit_wyscout_v5_runtime.py` plus tests: locked/no-sync no-site/Packaging bootstrap, pre-build admission prefix/process, closed input envelope, `.pth`/editable root, `L==I`, 35 executables, three aliases, source-complete pyc authority/classes, canonical manifest construction/result frame | offline constructor only; cannot write manifest/calculate build ID; R19 does not create script |
| 23 | rebuild entrypoint/master | future exact `scripts/rebuild_wyscout_v5.py`, locked/no-sync integration test, exact rebuild-input envelope, bounded result frame, projection reconstruction, invocation receipt only | only after manifest readback, one build-ID SHA-256, post-hash invocation construction, then rebuild-prefix creation; calls sole writers; R19 does not create script |
...
R19 retains the accepted R18 design without changing R18's accepted merits or
...
path, and every source/semantic/runtime closure. It refreshes only the truthful
operational bytecode snapshot and closes the no-write review harness; the stable
...
unchanged. R19 created or modified only this design and its assigned return under
