# W04 Wyscout v5 canonical schema and deterministic rebuild design — R20

Status: **implementation design for master and independent review; not self-approved**

This document replaces R19 in full. It is the standalone design for the local-only
W04 Wyscout Figshare v5 Bronze-to-Gold proof. The master accepted R19; that
acceptance is preserved. The valid independent R12 review found one P1 defect:
R19 required the `python` wrapper alias for every Class-E row, but the frozen
installed environment contains four deterministic Class-E wrappers whose exact
verified first line uses the contained `python3` alias. R20 corrects only that
executable-shebang authority, bumps the executable-census algorithm to v3 and the
enclosing code/environment manifest schema to v15, and preserves the no-write
review harness. It does not reinterpret R19, broaden wrapper selection, authorize
repair, or make root-bearing shebang bytes stable authority. Every passing,
master-reproduced R19 closure, every earlier passing closure, and every valid
passing independent finding remains binding without compression or substitution:
all source, rights, temporal, identity, football-product, coverage, quarantine,
path, serializer, environment, resource, exact executable/alias/bytecode, gate,
ownership, two-root, and two-commit ledger contracts remain binding. In
particular, the exact 119-pair field roster, strict UUID `ActorId`, complete
possession contract, and approved
`tests/contracts/test_wyscout_field_registry_authority.py` path remain unchanged.

All passing R19 closures remain unchanged: the current host's exact uv logical
...
17-resource closures remain binding. R20 changes no architecture, provider,
rights, dependency, lock, migration, network, storage root, ignore rule, remote,
deployment, product writer, child argv, or stable v4/v2/v2 process/bootstrap
schema. The only stable preimage change is the exact executable-census component
described in Section 8.5; its enclosing manifest schema is therefore v15.
...
| `code_manifest_schema_version` | string | exact `w04-code-environment-admission-v15`; stable |
...
| `manifest_schema_version` | string | exact `w04-code-environment-admission-v15` |
...
`python` is the default uv POSIX wrapper-shebang alias and `python3` is the one
separate exceptional wrapper-shebang alias admitted only by the exact constructive
selector in Section 8.5.2. They are distinct exact roles even though the admitted
`python3 -> python` chain reaches the same physical interpreter. `python3.12` and
every other spelling are forbidden in Class E and P. Both exact launches—
`uv run --locked --no-sync python -S -B
...
Admission records all three absolute alias paths, raw link texts, distinct lstat
identities, complete resolution chains, the operational locked/no-sync
`python -> python3` launch observation, resolved physical path, and the exact
per-row wrapper shebang. Stable identity binds the three alias basenames, both
root-independent safe relative chains (`python3 -> python` and
`python3.12 -> python`), the `python -> physical interpreter` role, the distinct
selected `python3 -> python -> physical interpreter` wrapper role, physical
interpreter identity/digest, and algorithm
...
strict UTF-8/LF and equal exactly one of two reviewed uv POSIX templates. The
templates differ only in the first-line alias selected by the constructive rule
that follows. The exact `python` template is:
...
The alias is not a free value. For a verified direct entry-point row, construct
the exact tuple

```text
(normalized_owner_name_and_version,
 exact_entry_point_name,
 exact_entry_point_group,
 exact_module_path_and_attribute)
```

and select `python3` if and only if the tuple is one of:
...
Every other Class-E tuple selects `python`. Class P always selects `python`.
Therefore exactly 29 Class-E rows plus the sole Class-P row use `python`, exactly
four Class-E rows use `python3`, and the complete text-wrapper split is exactly
30 `python` and four `python3`; Ruff remains the sole Class-W binary. The selector
is equality over all four tuple fields, not basename recognition, owner-only
recognition, target-only recognition, a set of interchangeable aliases,
`sys.executable`, realpath equality, first-existing lookup, fallback, or repair.

The selected first line must be exactly the contained admitted alias—not
`python3.12`, `/usr/bin/env`, an argument-bearing shebang, an external interpreter,
another root, or another alias. The remaining bytes are the exact deterministic
function of the verified target and are identical between the two templates. GUI
entries use the same reviewed POSIX template only on this frozen platform.

The four exact current-host `python3` rows are closed evidence:

| Owner / entry-point tuple | RECORD row | target | size | SHA-256 | RECORD URL-safe digest |
| --- | --- | --- | ---: | --- | --- |
| `detect-secrets==1.5.0 / detect-secrets / console_scripts / detect_secrets.main:main` | `../../../bin/detect-secrets` | `detect_secrets.main:main` | 380 | `16c1dcee3bcc2078fc6d4df7c0c85db6d043ab3bff5b40f8a13eea19e28aff3f` | `sha256=FsHc7jvMIHj8bU33wMhdttBDqzv_W0D4oT7qGeKK_z8` |
| `detect-secrets==1.5.0 / detect-secrets-hook / console_scripts / detect_secrets.pre_commit_hook:main` | `../../../bin/detect-secrets-hook` | `detect_secrets.pre_commit_hook:main` | 391 | `c3535b96dea57e7a88ab48961f74e51c07b019103b89a49c3f71da4cfbda5010` | `sha256=w1Nblt6lfnqIq0iWH3TlHAewGRA7iaScP3HaTPvaUBA` |
| `httpx==0.28.1 / httpx / console_scripts / httpx:main` | `../../../bin/httpx` | `httpx:main` | 366 | `7f7d4f633504d3f62f33335a9630e5bb4240989c9fb777b4a57e9d5c98fa394d` | `sha256=f31PYzUE0_YvMzNaljDlu0JAmJyft3e0pX6dXJj6OU0` |
| `pip-licenses==5.5.5 / pip-licenses / console_scripts / piplicenses:main` | `../../../bin/pip-licenses` | `piplicenses:main` | 372 | `b563dfd0133f2295a703e09a820fd4b133fd1d2c438150dc6c42ec7d62e8b52f` | `sha256=tWPf0BM_IpWnA-Cagg_UsTP9HSxDgVDcbELsfWLotS8` |

Each is one regular, non-symlink, non-hardlink-aliased file with mode `0o755`,
link count one, the exact size/bytes/digest/owner/RECORD row/target above, and
exact first line:
...
For each row, every byte after the first LF equals the deterministic template body
for its exact verified target. The current absolute line and complete actual
digest are operational evidence; the selected alias role and normalized template
bytes are stable evidence.
...
selected `python` shebang and remaining template bytes must pass before the alias
is admitted. Class P has no `python3` exception. No `pip3`, `pip3.12`, or basename
rule may generate a second class:
...
chain, selected alias role, and verification. For E and P only, admission first
requires the complete actual bytes to equal the exact selected template. It then
performs exactly one stable normalization:

```text
verified python first line:
  #!<project-root>/.venv/bin/python\n
stable first line:
  #!<W04_VENV_WRAPPER_PYTHON>\n

verified python3 first line:
  #!<project-root>/.venv/bin/python3\n
stable first line:
  #!<W04_VENV_WRAPPER_PYTHON3>\n
```

Every byte after the first LF is retained unchanged. The two tokens are distinct
and non-interchangeable. Normalization applies only after the constructive tuple
selector, exact alias chain, complete bytes, owner, entry-point group, target,
RECORD row, size, digest, mode, regular-file/no-link predicates, and class have all
passed. It does not normalize another shebang or substring, accept either alias,
realpath an arbitrary interpreter, select from `sys.executable`, fall back, or
repair. W has no root-bearing shebang; its exact byte digest/size/mode and
extracted/installed equality are stable unchanged.

Each stable row binds class, owner, RECORD row, entry-point group/target or P/W
authority, selected alias role, normalized template bytes, mode, and constructive
selector outcome so equal physical interpreter resolution cannot collapse
`python3` into `python`, P into E, or E/P into W. The combined version is
`w04-installed-executable-census-v3`. Its closed cardinalities are 35 total,
33 E, one P, one W, 21 owners, 29 E plus one P selecting `python`, and four E
selecting `python3`. Stable environment/build/two-root identity uses the 35 sorted
classed rows; operational receipts use actual rows. Distinct roots require
identical membership/classes/owners/groups/targets/selected-alias roles/normalized
bytes/modes and W bytes. Admission may read all 35 only before denial. From each
process's guard installation through its final recheck, open/read/mmap/execute of
every executable target is denied and audited. Zero reads and zero executions are
...
The R19 evidence snapshot observes 1,086 site pycs: 973 normal lexical names,
112 pytest-rewrite names, and the optional six orphan present. The 973 normal names
are 972 distribution-source mappings plus the one uv-bootstrap mapping. These
counts are a dated operational evidence snapshot, not an admission requirement for
this or another root and not a stable-map cardinality. Every later bounded
admission, runtime, or review records its own actual preflight inventory and
decomposition. A later actual total may differ because mapped caches may have been
created, omitted, or rewritten before that preflight; every file present at the
preflight must nevertheless classify under this source-derived map or one of the
four exact optional orphan predicates.

##### 8.6.2A Preserved R11 Packaging-cache incident evidence

The invalid independent R11 review imported installed Packaging modules without
Python bytecode denial and created the following exact eleven files before taking
the required site census. All eleven are regular files, mode `0o644`, link count
`1`, current magic `cb0d0d0a`, and have the exact installed source sibling shown.
All share modification epoch `1785412176`, rendered
`2026-07-30T12:49:36+0100`.

| exact pyc path | exact source sibling | size | SHA-256 |
|---|---|---:|---|
...
These files account for the eleven-cache delta captured in the R19 evidence
snapshot. Each is `SITE_DISTRIBUTION_NORMAL`: its exact normal grammar maps to its
present RECORD-owned Packaging source. The incident changed operational state and
invalidated R11's chain of custody; it did not change the stable source authority,
the four orphan predicates, the architecture, or any schema preimage. The files
remain evidence. No cleanup, repair, truncation, quarantine, environment
recreation, or other attempt to manufacture a prior state is permitted.
...
Actual pyc paths, bytes, hashes, sizes, modes, the actual bounded-run preflight site
and repository counts/decomposition, the R19 evidence snapshot, optional-orphan
presence, three UUIDs, descriptor numbers, device/inode values, and all three
absolute prefix paths are operational. The source-derived authority map, exact
four optional predicates, three-role prefix policy, exact bootstrap source rows,
and denial policy are stable. A second empty root may have different mapped pyc
paths/counts/bytes and omit any optional orphan; every present file must still
classify and stay unread. Root equality never requires operational inventory
equality.

For every bounded runtime or review, the admission baseline is the actual
read-only preflight inventory taken immediately before the bounded run, not a
hardcoded snapshot cardinality. The preflight enumerates every site and repository
pyc, classifies every file, and fails before the run if any file is unclassified,
unsafe, ambiguous, or violates its exact orphan predicate. The postflight repeats
the identical inventory construction and requires byte-for-byte equality with the
complete preflight inventory. Any creation, deletion, rename, content/header
mutation, mode/link/identity change, or other inventory drift during the run
invalidates it. Cleanup or repair can never turn a failed run into a success.
...
#### 8.6.5 Closed no-write independent-review harness

Before any design-review analysis, helper, or Python import, an independent
reviewer takes a read-only shell census and complete site/repository pyc inventory.
The inventory uses contained read-only shell operations and binds every relative
path, class, size, mode, link count, header/magic, source association where
applicable, and complete SHA-256. It records the actual class decomposition and
the delta, if any, from the R19 evidence snapshot above. Every present file must
classify; an unclassified file fails the review before merits adjudication.

Every review Python helper is invoked from the repository root through locked,
no-sync uv, begins with `PYTHONDONTWRITEBYTECODE=1`, and uses `python -B`.
Helpers that do not require installed packages use `python -S -B`. A helper that
does require installed packages sets both controls before interpreter startup and
verifies at its first instruction that `sys.dont_write_bytecode` is true and the
environment value is exactly `"1"` before importing another file-backed module.
No bare Python, unlocked uv, sync-capable uv, nested environment, or helper that
sets controls after startup is admitted.

Immediately after the last bounded review command, the reviewer repeats the
identical read-only shell census and inventory and requires exact identity with the
preflight inventory. Any pyc creation, deletion, mutation, rename, mode/link
change, or inventory drift invalidates the entire review and prevents a merits
verdict. The reviewer stops and preserves evidence; it does not delete or repair
files, run sync, purge caches, recreate the environment, or otherwise convert
failure into success. This harness is a review-procedure and chain-of-custody
contract only. Its actual inventory, counts, shell paths, clocks, and hashes are
operational evidence and never enter stable build identity.
...
from Section 8.5.1, the distinct default-`python` and selected-`python3` wrapper
roles, launch-time alias observation,
...
stable code/environment manifest:
  ...
  total 35-row classed executable census, exact constructive per-row alias selector,
    30/4 text-wrapper split, and two-token stable normalization
...
operational admission evidence:
  ...
  actual python/python3/python3.12 aliases, link text/chains, and exact per-row
    wrapper shebang
...
The stable manifest algorithm is `w04-code-environment-admission-v15`. Component
...
environment_digest = SHA256(canonical_json({
  ...
  "executable_census_digest": <35 reviewed classed stable rows under w04-installed-executable-census-v3, including the exact four-tuple selector, selected alias role, 30 python/4 python3 split, and two-token normalization>,
...
Version v15 differs from the prior manifest schema only because
`executable_census_digest` now commits to the v3 constructive selector, distinct
alias roles, exact four exceptional rows, exact 30/4 split, and two normalization
tokens. The other nineteen component keys, their value schemas, ordering, proof
rows, canonical manifest/result equality, 24-key projection intersection, exact
twenty component proofs, H1/H2 host-spelling perturbation, and Section 9 build-ID
construction are unchanged. A manifest, admission input/result, immutable
readback, projection equality, final recheck, health row, test fixture, or
two-root proof carrying the prior manifest schema literal fails; there is no stale
acceptance route.

The closed cardinality sequence remains exactly `16/8/10/25/25/20`: sixteen
common child-input keys, eight admission-input keys, ten rebuild-input keys,
twenty-five rebuild-invocation keys, twenty-five pre-build-projection keys, and
twenty component-proof rows. The projection and invocation retain exactly their
24 stable same-named values; v15 does not add, remove, rename, reorder, or relax a
field in any of those schemas.
...
class/owner/group/target/body/shebang/mode/normalization/census drift, a swapped,
added, or missing `python3` selector tuple, either alias used generically,
`python3.12`, `/usr/bin/env`, an argument-bearing or external shebang, owner/target
substitution, body mutation, fallback, or repaired wrapper, a stale executable
census or manifest schema version, unsafe/class-ambiguous/read pyc,
optional-orphan drift, incomplete source authority, any of the three aliases or
either relative alias chain drifting, interpreter/libpython/
...
`w04-code-environment-admission-v15` bytes have been written or confirmed,
...
- exact uv-bootstrap deny and normalized editable-root evidence;
- the total 35-row executable census/classes/owners/groups/targets and exact
  per-row selector, with 29 E plus one P normalized through
  `#!<W04_VENV_WRAPPER_PYTHON>\n`, exactly four E normalized through
  `#!<W04_VENV_WRAPPER_PYTHON3>\n`, byte-identical post-first-LF bodies, and exact
  W bytes;
...
- byte-identical `w04-code-environment-admission-v15` canonical manifest values,
  exact twenty component proofs, immutable readback equality, and no prior schema
  literal;
...
census with 33 E/one P/one W, 21 owners, the exact 29-E-plus-one-P `python` and
four-E `python3` selector split, exact three-alias interpreter closure,
source-complete site/repository
...
groups/targets, selected alias role, exact 30-`python`/four-`python3` split, both
normalization tokens, and rejection status for every forbidden alias/selector
case; the exact three-alias topology; source-complete pyc authority-map digest;
all four exact optional orphan predicates and each presence; the actual run
preflight site/repository census, cache-directory count, complete class
decomposition, and signed deltas from the R19 evidence snapshot (`1,086` site =
`972` distribution-normal + one uv-bootstrap normal + `112` pytest rewrite +
optional-six present; repository `58 pyc / 19 cache directories = 35 mapped
normal + 20 mapped pytest + migrations-env + migrations-foundation + PostgreSQL
source-absent inert orphans`); the complete preflight-inventory digest and the
byte-identical postflight digest; explicit zero creation/deletion/mutation drift;
...
| 21L | local-control implementer/master | future exact `scripts/launch_wyscout_v5.py` plus acyclic bootstrap digest, first-instruction guard, whole-repo pyc census, source-descriptor/closed child-input/result/diagnostics, three-prefix chronology, TOCTOU, manifest-write, exact pre-build projection, one build-ID SHA-256, and post-hash invocation tests | transient control only; R20 names but does not create script |
| 22 | admission implementer/master | future exact `scripts/admit_wyscout_v5_runtime.py` plus tests: locked/no-sync no-site/Packaging bootstrap, pre-build admission prefix/process, closed input envelope, `.pth`/editable root, `L==I`, 35 executables, three aliases, source-complete pyc authority/classes, canonical manifest construction/result frame | offline constructor only; cannot write manifest/calculate build ID; R20 does not create script |
| 23 | rebuild entrypoint/master | future exact `scripts/rebuild_wyscout_v5.py`, locked/no-sync integration test, exact rebuild-input envelope, bounded result frame, projection reconstruction, invocation receipt only | only after manifest readback, one build-ID SHA-256, post-hash invocation construction, then rebuild-prefix creation; calls sole writers; R20 does not create script |
...
1. Master completes implementation/readback, independent reviews, candidate
   ...
   registry remains pre-checkpoint. Candidate, gate, acceptance, and registry
   evidence must agree on `w04-installed-executable-census-v3` and
   `w04-code-environment-admission-v15`; a missing value or any prior schema
   literal blocks the ledger.
...
11. Prove exactly three distinct mode-`0o755` symlinks: `python ->` the exact
    ...
    resolving identically, exact distinct `python` and `python3` wrapper roles, and
    the locked/no-sync uv `python -> python3` launch observation operational only.
    Require each selected `python3` shebang to traverse the contained one-hop
    `python3 -> python` link before the frozen physical interpreter. Reject a
    missing or fourth alias, raw-target drift, cycle, extra hop, escape, inode
    alias, physical mismatch, interchangeable alias treatment, or incidental
    `sys.executable` equality.
12. Census exactly 35 `../../../bin` rows across 21 owners and exactly one class
    ...
    owner, entry-point group, target/authority, bytes/hash/size/mode, collision
    rule, stable row, and zero admission execution/rebuild read or execution.
    Exercise the exact four-tuple selector: exactly the four named Class-E rows
    select `python3`; the other 29 Class-E rows and sole Class-P row select
    `python`; Ruff is W. Require the exact 30/4 text-wrapper split, exact current
    four-row sizes/digests/RECORD values, deterministic bodies, both normalization
    tokens, `w04-installed-executable-census-v3`, and
    `w04-code-environment-admission-v15` throughout input, result, immutable
    manifest, equality checks, component proof, health, tests, projection, final
    recheck, and two-root evidence.
13. Reject a missing/extra/multiply classified bin row; an undeclared pip alias;
    ...
    script rewrite/`#!python` trigger; unsafe name; link/hardlink; and a swapped,
    added, or missing exceptional selector tuple; generic acceptance of either
    alias; `python3.12`; `/usr/bin/env`; shebang flags or arguments; an external or
    alternate interpreter/root; changed post-first-LF body; changed owner, group,
    RECORD row, or target; realpath-only selection; `sys.executable` selection;
    fallback; repaired/generated wrapper; wrong normalization token; stale
    executable-census version; or stale manifest schema version.
14. Build the stable authority map from every admitted RECORD-owned third-party
    ...
    Preserve the R19 operational evidence snapshot: site `1,086 = 972` mapped
    distribution normal + one mapped bootstrap normal + `112` mapped pytest +
    optional six orphan; repository `58` in `19` cache directories = `35` mapped
    normal + `20` mapped pytest + exactly three present source-absent inert orphans:
    ...
    non-empty alternate prefix. For the actual bounded run, take the complete
    read-only preflight inventory, require every present pyc to classify, record
    its actual site/repository class decomposition and delta from the R19 evidence
    snapshot, then require the complete postflight inventory to be byte-identical.
    The actual run total is never required to equal the R19 snapshot; creation,
    deletion, or mutation during the run fails, and cleanup cannot recover a pass.
    Prove another root may independently omit any of the four optional orphans and
    vary mapped pyc counts/paths/hashes without changing stable authority.
...
18. Before any independent design-review analysis or Python import, take the
    complete read-only shell site/repository pyc census and inventory. Invoke every
    Python helper only as locked/no-sync root uv with
    `PYTHONDONTWRITEBYTECODE=1` and `python -B`, using `python -S -B` whenever
    installed packages are unnecessary. For installed-package helpers, prove both
    controls at the first instruction before another file-backed import. Repeat
    the identical shell inventory postflight and require exact equality. Exercise
    a helper-created pyc, deletion, header/content mutation, mode/link change, and
    unclassified new file as review-invalidating failures. Prove no cleanup,
    repair, sync, cache purge, or environment recreation can convert failure into
    a review pass, and prove the harness remains review procedure rather than a
    stable build input.
...
R20 retains the accepted R19 design without changing R19's accepted merits or
self-approving the replacement. It preserves the existing strict UUID actor
primitive, complete possession predicate closure, approved field contract-test
path, the R19 operational bytecode evidence snapshot, its dynamic preflight rule,
and every source/semantic/runtime closure. It corrects only constructive executable
shebang authority: exact 30-`python`/four-`python3` wrapper selection,
`w04-installed-executable-census-v3`, and the resulting
`w04-code-environment-admission-v15` preimage. The stable
`w04-local-control-bootstrap-v4`, `w04-outer-environment-bootstrap-v2`,
`w04-child-environment-input-v2`, all child argv, cardinalities, projection,
build-ID, H1/H2, manifest/result, two-root, gate, ledger, and no-write review
contracts remain unchanged except for the exact v15 manifest literal and
executable component value. R20 created or modified only this design and its
assigned return under the project root; it created no parent-workspace report
path, directory, future script, or cleanup obligation. Implementation begins only
after the master and a separate independent reviewer accept this standalone
design.
