# W04 Wyscout v5 canonical schema and deterministic rebuild design — R17

Status: **implementation design for master and independent review; not self-approved**

This document replaces R16 in full. It is the standalone design for the local-only
W04 Wyscout Figshare v5 Bronze-to-Gold proof. R17 retains every passing,
master-reproduced R16 closure, every earlier passing closure, and every passing
independent R10 finding without compression or substitution: all source,
...
ownership, two-root, and two-commit ledger contracts remain binding. R16 received
the master candidate decision recorded by its review, then independent R10 returned
`REWORK` for two reproduced P1 findings. Neither decision is retroactively changed.
R17 corrects both returned defects. First, the current host's exact uv logical
launch, raw-link, and physical-path spellings remain mandatory operational
admission evidence but are absent from every stable identity. Stable uv authority
uses only named roles, a root-independent one-hop relative-link policy, and the
exact admitted physical bytes/version/mode/size. Second, each of the four semantic
authority routes is now a closed standalone protocol, beginning with an exact
119-path field route. The R16 acyclic outer/child environment,
...
Every actor is a strict nonempty NFC ASCII `ActorId` matching
`[A-Za-z0-9][A-Za-z0-9._:@/-]{0,127}`. Every clock is a truthful canonical UTC
instant under Section 8.0.5 and never a filesystem, Git, or report-rendering time.
For every route:
...
| --- | --- |
| `W04-FIELD-SEMANTIC-DECISION-01-R1` | `reports/reviews/W04/authorities/wyscout-field-semantic-decisions-v1.json`; `configs/schema/wyscout-v5-field-registry-v1.yaml`; `tests/contracts/test_w04_field_semantic_authority.py`; `reports/reviews/W04/returns/W04-FIELD-SEMANTIC-DECISION-01-R1.md` |
...
predicate. Each row has exactly:
...
Required and forbidden tags are disjoint. `CONTROL|RESTART` requires
`control_team_source=ACTION_TEAM` and `opens_control=true`.
`CONTESTED|DEAD_BALL|NON_CONTROL_ADMIN|UNMAPPED` requires
`control_team_source=NONE` and `opens_control=false`. The closed `policies` object
is exactly:
...
decision. Its canonical digest is the candidate digest. The review/acceptance use
the common exact schemas, route-fixed IDs, and null v1 supersession.
...
Tests reject overlapping predicates, unsorted/duplicate tags, a tag in both sets,
invalid decision/team/open combinations, label/name matching, a provider-native
claim, field/input drift, noncanonical candidate/review/acceptance, actor/clock
failure, any digest-edge failure, possession construction before acceptance, and
any dependency mismatch.
...
| 21L | local-control implementer/master | future exact `scripts/launch_wyscout_v5.py` plus acyclic bootstrap digest, first-instruction guard, whole-repo pyc census, source-descriptor/closed child-input/result/diagnostics, three-prefix chronology, TOCTOU, manifest-write, exact pre-build projection, one build-ID SHA-256, and post-hash invocation tests | transient control only; R17 names but does not create script |
| 22 | admission implementer/master | future exact `scripts/admit_wyscout_v5_runtime.py` plus tests: locked/no-sync no-site/Packaging bootstrap, pre-build admission prefix/process, closed input envelope, `.pth`/editable root, `L==I`, 35 executables, three aliases, source-complete pyc authority/classes, canonical manifest construction/result frame | offline constructor only; cannot write manifest/calculate build ID; R17 does not create script |
| 23 | rebuild entrypoint/master | future exact `scripts/rebuild_wyscout_v5.py`, locked/no-sync integration test, exact rebuild-input envelope, bounded result frame, projection reconstruction, invocation receipt only | only after manifest readback, one build-ID SHA-256, post-hash invocation construction, then rebuild-prefix creation; calls sole writers; R17 does not create script |
...
R17 retains every passing R16 closure and every passing independent R10 finding
without changing the recorded R16 master-candidate or R10 `REWORK` decisions and
without self-approval. R17 created or modified only this design and its assigned
