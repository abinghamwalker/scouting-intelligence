# W04 Wyscout v5 canonical schema and deterministic rebuild design — R10

Status: **implementation design for master and independent review; not self-approved**

This document replaces R9 in full. It is the standalone design for the local-only
W04 Wyscout Figshare v5 Bronze-to-Gold proof. R10 retains every R9 and earlier
...
ownership, and ledger contract. It corrects only the remaining stage-0/build-ID
ordering cycle, the measured repository bytecode split and inert PostgreSQL
orphan, and the requirement that stable bytecode authority be complete over
admitted source rather than frozen to current pyc inventory, plus the measured
third `python3.12` interpreter alias. It changes no
...
after it. Both start exactly the canonical venv interpreter with `-S -B`. The sole
...
1. **Pre-build admission process.** Before any build ID exists, the external
   ...
   admitted environment value, and launches exactly:
   ...
   the canonical bytes plus component proofs to the admitted external launcher
...
3. **Rebuild process.** With `<build_id>` now available, the launcher safely creates
   ...
   controls, and launches the repository-owned rebuild entry point with `-S -B`.
...
repository-owned entry-point bytes, environment, selected empty alternate prefix,
and no-network controls are externally hashed/admitted before each respective
process. `-S` and `-B` are mandatory in both. At the first Python instruction,
...
complete resolution chains, the operational `uv run python -> python3` spelling,
...
each Section 8.0 Python process exists, the external launcher enumerates the exact
...
list, never frozen to 56 repository rows, and never expanded by an orphan.

Stable identity binds algorithm `w04-preexisting-pyc-enumerate-deny-v3`, current
interpreter cache tag/magic, exact normal/pytest grammars and pytest version, the
complete source-authority map above, traversal-root roles, both exact optional
orphan predicates below, and the no-cleanup/zero-read/zero-change policy. Source
...
2. `REPOSITORY_PYTEST_REWRITE` — its exact pytest grammar maps to one existing
   repository-code-manifest-owned `.py` source row.
3. `REPOSITORY_POSTGRES_OPTIONAL_INERT_ORPHAN` — optionally exactly
   ...
   semantic authority. Its absence in a fresh root is valid; changed present
   bytes/header/mode, a source appearing, a different orphan, or a second
   repository orphan fails.

The current first-root repository observation is 56 pyc in 17 `__pycache__`
directories: exactly 35 normal files mapped to present admitted sources, exactly
20 `cpython-312-pytest-9.1.1` files mapped to present admitted sources, and the
optional PostgreSQL inert orphan present. The former R9 `36 normal + 20 pytest`
statement incorrectly counted this source-absent orphan as mapped normal bytecode.
No repository pyc outside the admitted source map or exact optional PostgreSQL
predicate is accepted; whole-repository traversal ensures such a file is denied
and fails admission rather than broadening authority.
...
bind their verified source row in the operational receipt. The two optional orphan
predicates are stable policy, but orphan presence and the observed present file are
operational; neither orphan enters the source map.
...
presence of either exact optional orphan, and inventory digests are operational
evidence only. Across equal stable inputs, roots may contain different numbers,
paths, and bytes of mapped normal/pytest pyc and may omit either optional orphan;
the complete source map and orphan predicates remain equal. Thus current 1,075-site
and 56-repository counts, or their path/hash lists, are never two-root equality
...
orphan, unclassified pyc, second orphan, any pyc read, in-place mutation, new pyc,
...
The stable manifest algorithm is `w04-code-environment-admission-v7`. Component
...
exists; the external launcher writes or confirms the immutable path above and only
...
prefix reuse/non-emptiness, and early build-ID formation.

## 9. Build identity, deterministic bytes, and two-root proof

Canonical build-input version is `w04-wyscout-build-id-v8`. It contains:
...
- the source-complete site/repository pyc authority map plus exact optional-six and
  optional-PostgreSQL inert-orphan predicates, never operational root-bearing
...
- the complete source-derived site/repository pyc authority map and both optional
  orphan predicates (not actual pyc bytes/inventories or current counts);
...
pyc inventories may differ only in their documented operational fields. Either
...
two-process ordering with both exact empty alternate prefixes, exact resources,
sole writers, quarantine reconciliation, and two-root equality.
...
both optional orphan predicates/presence; first-root operational site/repository
class inventories including repository `35 normal + 20 pytest + PostgreSQL orphan`;
admission/rebuild process IDs and prefix roles; per-process empty-before/after and
zero-read/no-change proof; loaded subset `R`; uv/alias/interpreter/stdlib/resources;
...
| 22 | admission implementer/master | admission module/script/tests including no-site/Packaging bootstrap, pre-build admission prefix/process, `.pth`/editable root, `L==I`, 35 executables, three aliases, source-complete pyc authority/classes | offline algorithm only |
...
| 25 | code-manifest admit/master | exact immutable code manifest and admission report/return | after code freeze, before build ID |
...
9. Before any build ID, sample `admission_run_id`; create/prove empty only
   `data/working/wyscout/v5/.staging/admission/admission_run_id=<uuid>/runtime-pycache/`;
   set its absolute path, `PYTHONDONTWRITEBYTECODE=1`, and `-S -B` before stage-0
   creation. Prove no site/editable root, `_virtualenv`, coverage, `.pth`,
   executable, or pyc is observed. Byte-admit only Packaging 26.2, freeze the
   immutable code manifest, then compute build ID. Start a separate `-S -B` rebuild
   process using only the proved-empty
   `.staging/<build_id>/<run_id>/runtime-pycache/`. Reject an unknown-build stage-0
   path, process/prefix reuse, either non-empty prefix, or any in-place pyc change.
...
11. Prove exactly three distinct mode-`0o755` symlinks: `python ->` the exact
    ...
    the `uv run python -> python3` spelling operational only. Reject a missing or
...
14. Build the stable authority map from every admitted RECORD-owned third-party
    ...
    six/PostgreSQL orphan predicates. Record the first-root observations: site
    1,075 = 961 mapped distribution normal + one mapped bootstrap normal + 112
    mapped pytest + optional six orphan; repository 56 in 17 cache directories =
    35 mapped normal + 20 mapped pytest + optional exact
    `src/scouting/storage/__pycache__/postgres.cpython-312.pyc` orphan. Reject
    unsafe/ambiguous/wrong-tag/magic/pytest/source ownership, changed present or
    second/different orphan, pyc read/change/new file, or non-empty alternate
    prefix. Prove another root may omit either orphan and vary mapped pyc
    counts/paths/hashes without changing stable authority.
