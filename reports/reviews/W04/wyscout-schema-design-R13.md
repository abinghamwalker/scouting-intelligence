# W04 Wyscout v5 canonical schema and deterministic rebuild design — R13

Status: **implementation design for master and independent review; not self-approved**

This document replaces R12 in full. It is the standalone design for the local-only
W04 Wyscout Figshare v5 Bronze-to-Gold proof. R13 retains every R12 and earlier
closure without compression or substitution: all source, rights, temporal,
...
two-commit ledger contracts remain binding. It closes only the returned R12 master
findings: truthful pre-guard encoding-source startup under a distinct control
prefix, feasible inherited launcher/child source descriptors with honest race
limits, and exhaustive typed child-result payloads. It changes no architecture,
...
The fixed outer environment is the Section 8.8 allowlist plus exactly:

```text
PYTHONDONTWRITEBYTECODE = "1"
PYTHONNOUSERSITE = "1"
PYTHONHASHSEED = "0"
PYTHONPYCACHEPREFIX = <absolute reviewed control prefix>
PYTHONPATH = absent
COVERAGE_PROCESS_START = absent
COVERAGE_PROCESS_CONFIG = absent
W04_CHILD_ROLE = absent
W04_ENTRYPOINT_SOURCE_FD = absent
W04_RESULT_FD = absent
W04_RESULT_NONCE = absent
W04_LAUNCHER_SOURCE_FD = <strict decimal inherited descriptor>
W04_BOOTSTRAP_TUPLE_B64 =
  exact unpadded base64url(canonical UTF-8 JSON bootstrap tuple)
```

`fixed_environment_digest` covers stable values with operational
`control_run_id`, absolute prefix spelling, and descriptor number replaced by the
literal role tokens `<CONTROL_PREFIX>` and `<LAUNCHER_SOURCE_FD>`. The exact
transport environment is separately hashed operationally without replacement.
Unknown behavior-affecting variables fail.
...
3. samples the admission and rebuild UUIDs and distinct result nonces;
4. safely creates/proves empty the admission and rebuild prefixes in their required
   order;
5. preopens and transports each exact child source descriptor as Section 8.0.4;
6. constructs each fixed child environment and launches the two unchanged argv;
7. validates result frames, diagnostics, EOF, exit, descriptor, path, pyc, and
   prefix postconditions;
8. atomically writes/confirms only the immutable canonical code manifest returned
   by admission; and
9. only after manifest readback equality calculates the Section 9 build ID and
   supplies it to rebuild.
...
Admission uses the admission prefix before a build ID exists. Rebuild uses only the
build-scoped prefix after immutable manifest publication and build-ID calculation.
...
Publication order is: admission frame digest/EOF; descriptor-final binding;
diagnostic EOF; exit `0`; admission path/pyc/prefix/launcher postchecks; exact
manifest-byte and proof validation; immutable atomic manifest write/confirm;
readback equality; build-ID calculation; rebuild-prefix creation; rebuild launch.
Any failure leaves no build authority and is not repaired or cleaned.
...
#### 8.0.5 Canonical JSON and exhaustive closed child-result schemas

Payload JSON is canonical UTF-8 with no BOM. Objects serialize keys in increasing
...
`true`/`false`. Null is forbidden everywhere. Base64url is RFC 4648 URL alphabet
...
grammar. UUIDs are canonical lowercase v4,
...
|---|---|---|
| `child_environment_sha256` | string | SHA-256 of canonical JSON of the complete actual child environment, including actual FD decimals and nonce |
...
| `manifest_schema_version` | string | exact `w04-code-environment-admission-v10` |
...
#### 8.0.6 Checkpoint detection and honest same-trust-domain residual
...
stable code/environment manifest:
  ...
  exact local launcher row, role, ordered argv, accepted bootstrap tuple,
    encoding-source verifier, outer prefix policy, and inherited source descriptor
  exact locked/no-sync outer/admission/rebuild argv, roles, three prefix policies,
    entry-point paths/bytes, and inherited source descriptors
  exact bounded child-result frame plus exhaustive v2 payload schemas and
    sole-control ownership
...
The stable manifest algorithm is `w04-code-environment-admission-v10`. Component
...
environment_digest = SHA256(canonical_json({
  ...
  "local_launcher_control_digest": <launcher row, role, exact argv, bootstrap tuple, encoding-source verifier, control-prefix policy, and inherited launcher-source descriptor policy>,
  "process_launch_contract_digest": <the exact outer and two exact child ordered locked/no-sync argv, three roles, three prefix policies, and source-descriptor code rows>,
  "child_result_contract_digest": <frame, exhaustive v2 role payload schemas, inherited entrypoint descriptors, diagnostics, timeout/EOF checks, and sole-control ownership>,
...
tests cover launcher/bootstrap tuple/role/argv/environment/encoding-source/
...
canonical-JSON/closed-schema/nonce/role/code identity/digest/EOF/timeout/exit/order
...
Canonical build-input version is `w04-wyscout-build-id-v11`. It contains:
...
- exact local-control launcher row, role, ordered argv, accepted bootstrap tuple,
  encoding-source first-instruction verifier, inherited source-descriptor policy, bounded child-result contract, and sole-control
  ownership;
- exact root-independent process-launch contract: the outer and both child ordered
  ...
  launcher/child inherited source-descriptor contracts, and all three
...
nonces/diagnostic bytes, device/inode observations,
...
- exact normalized launcher bytes/row/bootstrap/encoding-source verifier,
  three-prefix/source-descriptor and exhaustive child-result/ownership contracts;
...
| 21L | local-control implementer/master | future exact `scripts/launch_wyscout_v5.py` plus bootstrap, first-instruction guard, whole-repo pyc census, source-descriptor/child-result/diagnostics, three-prefix, TOCTOU, manifest-write and build-ID-order tests | transient control only; R13 names but does not create script |
| 22 | admission implementer/master | future exact `scripts/admit_wyscout_v5_runtime.py` plus tests: locked/no-sync no-site/Packaging bootstrap, pre-build admission prefix/process, `.pth`/editable root, `L==I`, 35 executables, three aliases, source-complete pyc authority/classes, canonical manifest construction/result frame | offline constructor only; cannot write manifest/calculate build ID; R13 does not create script |
| 23 | rebuild entrypoint/master | future exact `scripts/rebuild_wyscout_v5.py`, locked/no-sync integration test, bounded result frame, invocation receipt only | after manifest/build ID; calls sole writers; R13 does not create script |
...
9. From the repository root invoke only exact
   ...
   Have the launcher alone sample distinct admission/rebuild UUIDs and create the
   two retained child prefixes. For each unchanged exact child argv, preopen the
   ...
   build ID/start rebuild. Reject unknown-build admission paths, flag/argv/prefix
...
R13 closes the returned R12 master findings and retains the returned independent R8 closures without self-approval. Implementation
