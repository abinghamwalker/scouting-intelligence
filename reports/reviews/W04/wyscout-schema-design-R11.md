# W04 Wyscout v5 canonical schema and deterministic rebuild design — R11

Status: **implementation design for master and independent review; not self-approved**

This document replaces R10 in full. It is the standalone design for the local-only
W04 Wyscout Figshare v5 Bronze-to-Gold proof. R11 retains every R10 and earlier
...
ownership, and ledger contract. It corrects only R10's process-launch boundary by
using exact locked/no-sync uv argv and exact repository entry-point paths, and its
stale final revision-lineage sentence. It changes no
...
operational pyc, and hash exact regular source/extension bytes. Stage 0 is exactly
`uv run --locked --no-sync python -S -B scripts/admit_wyscout_v5_runtime.py`;
...
The stable manifest algorithm is `w04-code-environment-admission-v8`. Component
...
generated-script execution, entry-point byte/role drift, and early build-ID
formation.

## 9. Build identity, deterministic bytes, and two-root proof

Canonical build-input version is `w04-wyscout-build-id-v9`. It contains:
...
argv and repository entry points, exact resources, sole writers, quarantine
...
zero-read/no-change proof; both exact ordered argv and entry-point path/byte/role
rows; uv locked/no-sync enforcement and observed physical digest/alias; loaded
...
| 22 | admission implementer/master | future exact `scripts/admit_wyscout_v5_runtime.py` plus tests: locked/no-sync no-site/Packaging bootstrap, pre-build admission prefix/process, `.pth`/editable root, `L==I`, 35 executables, three aliases, source-complete pyc authority/classes | offline algorithm only; R11 does not create script |
| 23 | rebuild entrypoint/master | future exact `scripts/rebuild_wyscout_v5.py`, locked/no-sync integration test, invocation receipt only | after manifest/build ID; calls sole writers; R11 does not create script |
...
9. Before any build ID, sample `admission_run_id`; create/prove empty only
   ...
   `uv run --locked --no-sync python -S -B scripts/admit_wyscout_v5_runtime.py`.
   Prove the entry-point bytes/role, no site/editable root, `_virtualenv`, coverage,
   `.pth`, executable, or pyc is observed, and independent `L==I` still passes.
   Freeze the immutable code manifest, then compute build ID. Start a separate
   process only with exact
   `uv run --locked --no-sync python -S -B scripts/rebuild_wyscout_v5.py` and the proved-empty
   `.staging/<build_id>/<run_id>/runtime-pycache/`. Reject an unknown-build stage-0
   path, missing/reordered flag, plain `uv run`, sync/reconciliation, alternate
   argv/interpreter/entry point, generated script, site startup, process/prefix
   reuse, either non-empty prefix, or any in-place pyc change.
...
R11 closes the returned R10 master findings without self-approval. Implementation
